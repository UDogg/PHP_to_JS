...
class PartnerAdminUserSeeder extends Seeder
{
    /**
    ...
    {
        //
        $usertype = DB::table('usertypes')->select('id')->where('Identifier', 'Partner')->first();
        $employeeCode = 2222;
        $lob = DB::table('lob_master')->select('id')->whereIn('lob', ['CAR','GCV','PCV','health','travel','super top up','BIKE','MISC','term','investment'])->get();
        $lastUserId = DB::table('users')->select('id')->where('name',credential_encrypt('Partner Admin'))->first();
        $role = DB::table('roles')->select('id')->where('name', 'Partner Admin')->first();


        if(empty($lastUserId))
        {
            $usercreate = DB::table('users')->insert([
                'name' => credential_encrypt('Partner Admin'),
                'email' => credential_encrypt('Partner_admin@fyntune.com'),
                'password' => Hash::make('Admin@123'),
                'username' => credential_encrypt('Partner_admin'),
                ...
                'account_holder_name' => credential_encrypt('Partner Admin'),
