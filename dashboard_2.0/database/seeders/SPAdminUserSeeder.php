...
class SPAdminUserSeeder extends Seeder
{
    /**
    ...
    {
        $usertype = DB::table('usertypes')->select('id')->where('Identifier', 'SP')->first();
        $employeeCode = 2222;
        $lob = DB::table('lob_master')->select('id')->whereIn('lob', ['CAR','GCV','PCV','health','travel','super top up','BIKE','MISC','term','investment'])->get();
        $lastUserId = DB::table('users')->select('id')->where('name',credential_encrypt('SP Admin'))->first();
        $role = DB::table('roles')->select('id')->where('name', 'SP Admin')->first();


        if(empty($lastUserId))
        {
            $usercreate = DB::table('users')->insert([
                'name' => credential_encrypt('SP Admin'),
                'email' => credential_encrypt('sp_admin@fyntune.com'),
                'password' => Hash::make('Admin@123'),
                'username' => credential_encrypt('sp_admin'),
                'mobile' => credential_encrypt('1234567890'),
                'address' => credential_encrypt('Mumbai'),
                'pincode' => credential_encrypt('401107'),
                'gender' => 'M',
                'status' => 'Y',
                'is_admin' => 'Y',
                ...
                'account_holder_name' => credential_encrypt('Sp_Admin'),
